...
- `companionResources` 只有满足 `unlock` 或默认关系门槛后才会进入 prompt、右侧局势、`availableCompanionModifiers` 和同伴行动线索；玩家可在推荐动作里直接看到“请某人协助……”，也可在检定卡点选或输入“请某人帮忙/投入协助名”，掷骰后扣除协助次数，并结算检定修正、证据可信度、后果解除、时钟变化、`cost.trust`、`cost.time` 与可能代价。同伴协助应绑定真实 `characterId`；生成、导入或旧档兼容回填时只允许按当前场景唯一角色名修复，重名、模糊或资源名同时提到多个角色时不能猜测绑定。资源未声明任何 `unlock` 时，运行时默认要求绑定 NPC 对玩家 `trust >= 10`；若剧本明确需要开局可用，必须写 `unlock:{ immediate:true }`、`unlock:{ always:true }` 或 `unlock:{ trustAtLeast:0 }`。资源可声明 `scope:"present" | "remote" | "pledged"`：`present` 必须 NPC 当前在场才可见和可用；`remote` 用于远程背书、资料或渠道；`pledged` 用于已承诺但稍后兑现的帮助。`immediate/always` 只绕过解锁门槛，不绕过 `present` 在场要求。同一次检定结算中，同一个 `resourceId` 即使因旧存档或 UI 异常重复出现在 modifiers，也只能扣除一次 uses 和代价。`unlock.revelationIds` 默认要求对应结论已 `confirmed`，不能因 `suspected` 过早公开同伴底牌；需要怀疑阶段解锁时必须显式声明。
...
- 检定卡中的失败缓冲物支持 `clock_resist`、负向 `clock_delta` 和负向 `world_tension`。玩家选中后，纯缓冲物只在部分成功、失败推进或大失败且实际缓解时钟/紧张度时消耗；若结果成功或没有可影响目标，不得扣除。缓冲生效后必须写入 `checkData.failureBuffers`、系统消息和事件日志，DM 续写必须能看到“资源已经缓解了哪项压力”。
...
- 获得、装备和直接使用物品时必须产生可回顾的用途反馈：任务物说明可调查/询问/查看，装备说明常驻效果和适用行动，消耗品说明直接使用或检定投入方式。反馈只能解释资源用途，不能把物品描述成万能解法或强迫主线。
...
- 自由行动命中当前 `clueGraph.stages[currentStage].actions` 时，除写入玩家知识和证据外，还必须推进到下一线索阶段并刷新右侧推荐；同一阶段的重复自由行动不能继续刷阶段。
...
- 输入“购买补给”“购买医疗包”“购买零件包”“购买短剑”“购买护甲”“购买工具包”“购买扫描仪”“购买伪装工具包”“购买追踪工具包”，会按固定价格扣金币并通过 `WorldEngine.grantInventoryItem()` 添加基础消耗品或装备；金币不足、重复购买已有非消耗装备，或背包已满且无法合并时只写系统提示，不扣金币、不改变背包。
...
- 2 级起，升级会通过最高属性自动派生“行动倾向”，例如技术调查、敏锐直觉或社交手腕。倾向进入成长/准备提示，并在匹配的行动预览中降低风险和 DC；它不能绕过检定、证据、物品消耗或挑战进度。
...
- API 成功返回但可见叙事为空时，不能把空白角色/DM 消息写入 transcript；必须移除流式占位，追加 `【AI空回复】` 系统提示，写入 `eventLog` 和 `scene.aiDiagnostics`，供玩家重试、继续输入或后续复盘排障。
...
- 主输入框下方的建议 chips 只作为可改写灵感，不代表必须点选的模式；idle 状态应优先结合失败预警、紧急时钟、最近后果、当前挑战、关键未知、地点和当前 NPC 生成自然句子，主线目标只能作为其中一个可选方向，不能长期占满首屏建议。
- `WorldEngine.getPressurePrompt()` 负责把失败预警、紧急时钟、最近后果和敌方反制转成短行动建议；无压力时不额外刷存在感。
...
- 低/中严重度后果如果有明确补救意图（道歉、解释、清理、稳定、修复、降低风险等），也可由低风险自由行动解除；高/关键后果不能被自由行动跳过，必须依赖检定、资源、同伴协助或正式挑战。
...
- 让不在场 NPC 知道私密对话。
- 让普通 NPC prompt 读取全局线索真相、隐藏失败条件、未知关键结论或未来剧情节拍。
- 把隐藏时钟具体名称暴露给玩家。
- 用系统私密事实替代玩家调查。

实现要求：

- `PromptBuilder.build()` / `buildGroup()` 必须以普通 NPC 视角调用规则层：可注入玩家属性、任务、物品、玩家已知情报、可见线索、可见失败风险、当前公开挑战和当前 NPC 自身私密扮演资料。
- `PromptBuilder.buildDMNarration()` 才能注入 `【线索图 · 私密结构】`、完整失败状态、未知关键结论和完整剧情弧节拍；同时必须注入 `【玩家已知情报】`，提醒 DM 区分内部真相和玩家已知。
- NPC prompt 中的剧情弧只能使用 `【剧情节奏 · 可感知层】`，不得包含未来 beat 的 condition/action。
...
- 首屏固定展示“当前态势”：地点、当前目标、最重要压力和最近风险。
- 当前挑战和 2-3 个主要行动排在历史记录、证据列表和完整压力详情之前。
- 可用准备、同伴协助、剧情上下文、压力后果、线索未知、记录回顾使用可展开分组；关键失败临界或高危时钟可以默认展开。
- 冒险回顾、完整对话记录和最近事件属于“记录与回顾”，不得平铺插在实时局势之前。
...
- 多维关系进入计策风险。
- 情报作为计策资源。
- 装备和非消耗任务物品进入检定修正；消耗品和已解锁同伴协助先在检定卡展示为可选资源，玩家点选后随掷骰消耗。同伴协助可用 `trustAtLeast`、`evidenceTags`、`knowledgeTags` 或 `revelationIds` 控制逐步公开；未写 `unlock` 的同伴协助默认需要 `trust >= 10`，不能开局直接泄露。
