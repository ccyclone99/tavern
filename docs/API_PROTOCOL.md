...
| `factionsUpdate` | 更新势力（name/attitude/power/description/leverage），单条补丁最多 20 条、场景总势力最多 40 个；变化会写入事件日志并提示局势 tab |
...
| `locationUpdate` | 新增或更新地点（id/name/description/connections/alertLevel），单条补丁最多 20 条、场景总地点最多 80 个；变化会写入事件日志并提示地图/局势 tab |
