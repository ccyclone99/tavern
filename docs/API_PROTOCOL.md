...
- 物品标记和 `<state_update>.itemAdd` 必须调用 `WorldEngine.grantInventoryItem()`、`removeInventoryItem()`、`equipInventoryItem()` 或 `unequipInventoryItem()`，不能直接改写 `scene.inventory`、`scene.equipment` 或 `scene.equipmentRefs`。
