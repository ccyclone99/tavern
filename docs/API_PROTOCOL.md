...
[check:属性名|DC值]
```
触发交互式检定卡：属性名可用中文（力量/敏捷/体质/智力/感知/魅力）或英文。玩家点击“掷骰”后，系统计算 D20 + 属性调整值 vs DC。
...
| `characterUpdates` | 更新角色关系（characterId/affectionDelta/trustDelta/suspicionDelta/fearDelta/debtDelta/leverageAdd/memoryAdd/mood/secret） |
| `clockUpdate` | 新增或推进时钟（id/name/value/delta/max/visibility/trigger/reason） |
| `storyArcUpdate` | 推进剧情弧（title/id/phase/synopsis/advance/advanceBy/currentBeat/reason） |
...
| `itemAdd` | 添加物品（name/description/type/quantity/effects/uses/tags） |
| `locationUpdate` | 更新地点 |
