...
- 主线任务全完成 → 调用 `WorldEngine.checkVictory()`；先补发未结算主线奖励，奖励被背包阻塞时继续保持 `playing`
...
- 物品标记和 `<state_update>.itemAdd` 必须调用 `WorldEngine.grantInventoryItem()`、`removeInventoryItem()`、`equipInventoryItem()` 或 `unequipInventoryItem()`，不能直接改写 `scene.inventory`、`scene.equipment` 或 `scene.equipmentRefs`。移除、装备、卸下、使用和出售物品时优先使用物品 id；只给名称时必须在当前背包中唯一，重名或模糊不唯一则跳过并提示，不能默认操作第一个同名物品。
- `<state_update>.questsUpdate`、`factionsUpdate`、`locationUpdate` 和 `scene.worldTensionDelta` 必须分别调用 `WorldEngine.applyQuestUpdates()`、`applyFactionUpdates()`、`applyLocationUpdates()` 与 `addWorldTension()`；规则层不可用时跳过并报警，不做降级直写。

### 角色/世界标记
```
[new_char:角色名|emoji|公开外貌|公开性格|开场白|信条|价值排序|底线1、底线2|动机1、动机2|恐惧1|秘密1|筹码1、筹码2]   # 新角色登场，后 7 项可选
[char_exit:角色名|原因]                    # 角色退场；角色名必须在当前场景内唯一
```
...
- 动态角色缺少扩展字段时，客户端会补齐保守的信条、底线、动机、恐惧、秘密和筹码，并写入 `profile.hiddenFacts`。动机、恐惧、秘密、筹码属于私密设定，只能通过调查、关系、检定或剧情事件逐步解锁。
...
<state_update>
{
  ...
  "discoveryUpdate": [{ "characterId": "char_xxx", "characterName": "塞拉斯", "factId": "secret_0_abcd", "factTitle": "未公开秘密", "state": "hinted", "evidence": ["..."] }],
  "intelAdd": [{ "text": "...", "source": "...", "reliability": "rumor" }],
  "factionsUpdate": [{ "name": "商会", "attitude": -10, "power": 60 }],
  "characterUpdates": [{ "characterId": "char_xxx", "characterName": "塞拉斯", "suspicionDelta": 10 }],
  ...
  "clueUpdate": [{ "id": "clue_cargo", "clueTitle": "货舱异响", "status": "active", "evidenceAdd": ["货舱异响"] }],
  ...
  "npcAgendaUpdate": [{ "characterId": "char_xxx", "characterName": "塞拉斯", "currentPlan": "监控玩家接触过的证人", "locationId": "bridge", "status": "away", "contact": "message", "canContact": true }],
  ...
  "revelationUpdate": [{ "id": "rev_smuggling", "conclusion": "走私链成立", "status": "confirmed", "reason": "证据链闭合" }],
  "flowGraphUpdate": { "nodes": [{ "id": "node_cargo", "status": "resolved" }], "revelations": [{ "id": "rev_smuggling", "status": "confirmed" }] },
  "questsUpdate": [{ "questId": "quest_main", "questName": "寻找真相", "objectiveIdx": 0, "objectiveText": "确认货舱异常", "status": "completed" }],
...
| 顶层字段 | 允许的子操作 |
|---------|-------------|
| `strategies.create` | 创建计策（title/goal/phase/risk/progress 等）；单条补丁最多 4 条，场景总计策最多 24 条 |
| `strategies.update` | 更新计策（必须带 id）；单条补丁最多 12 条，字段会经 `StrategyManager.normalizeStrategy()` 限幅 |
| `knowledgeAdd` | 添加玩家知识账本条目（subjectType/subjectId/level/text/source/reliability/tags） |
| `discoveryUpdate` | 更新 NPC 隐藏档案解锁状态（characterId/characterName + factId/factTitle/factType/hint/truth + state/evidence）。角色引用规则与 `characterUpdates` 相同；档案槽优先用 factId，缺失或未命中时只按该 NPC 下唯一标题、类型、hint 或 truth 片段匹配，重名或模糊不唯一则跳过 |
| `intelAdd` | 兼容旧协议的添加情报（text/source/reliability: rumor\|confirmed\|false），单条补丁最多 10 条；系统会裁剪、去重、保留最近 120 条并同步写入知识账本 |
| `factionsUpdate` | 更新势力（name/attitude/power/description/leverage），走 `WorldEngine.applyFactionUpdates()`；单条补丁最多 20 条、场景总势力最多 40 个；变化会写入事件日志并提示局势 tab |
| `characterUpdates` | 更新角色关系（characterId/characterName/name/actorName/targetName + affectionDelta/trustDelta/suspicionDelta/fearDelta/debtDelta/leverageAdd/memoryAdd/mood/secret）。`characterId` 优先；缺失或未命中时可按当前场景唯一角色名匹配，重名或模糊不唯一则跳过。单条补丁最多 30 个角色；每个角色单次最多追加 8 条筹码、8 条共同记忆、1 条私密秘密，字段会截断去重。公开关系变化会写入事件日志并提示详情/局势 tab；`secret` 只进入 NPC 私密设定，不直接展示给玩家。旧字段 `relationshipUpdate` 会兼容到同一处理路径，但新输出应使用 `characterUpdates` |
| `clockUpdate` | 新增或推进时钟（clockId/id 优先；也可按唯一 clockTag/tag 或唯一 name/clockName/title 匹配；重名或同 tag 不唯一则跳过。新增时至少提供 id/name/tag 之一；字段包括 value/delta/max/visibility/trigger/reason） |
| `storyArcUpdate` | 推进剧情弧（storyArcId/id 优先；也可按唯一 title/name/arcTitle 匹配；重名或模糊不唯一则跳过。字段包括 phase/synopsis/advance/advanceBy/currentBeat/reason；推进 beat 必须带 reason） |
| `storyPhaseUpdate` | 推进剧情阶段（storyPhaseId/phaseId/id 优先；也可按唯一 title/name/phaseTitle 或目标文本匹配；重名或模糊不唯一则跳过。字段包括 status/activate/reason/failForward/alternative/outcome/cost/bypassCost/costs/goldCost/worldTensionDelta/clockDelta；激活下一阶段或完成 active 阶段必须满足阶段闸门，结构化绕过代价会真实扣资源/写后果） |
| `clueUpdate` | 更新线索链（id/clueId/title/clueTitle/name/subjectName + status/currentStage/evidenceAdd/reason）。id 优先；缺失或未命中时可按唯一标题或 subjectName 匹配，重名或模糊不唯一则跳过。找不到且给出明确 id/title 时仍可创建新线索链 |
| `failureStateUpdate` | 触发或禁用剧本失败状态（failureId/id 优先；缺失或未命中时可按唯一 title/name/failureTitle 匹配；重名或模糊不唯一则跳过，不能猜测触发或禁用哪个失败条件。字段包括 status/title/severity/message/aftermath/hint/counterplay/recoverable/trigger） |
| `counterStrategyUpdate` | 新增或更新 NPC 反制（counterStrategyId/counterId/id 优先；也可按唯一 title/name/counterTitle，或唯一 actorId/actorName/target 组合匹配；不唯一则跳过。字段包括 status/visibility/progress/exposure/counterplay） |
| `npcAgendaUpdate` | 更新 NPC 日程与在场状态（characterId/characterName/name/actorName + currentPlan/priority/schedule/offscreenActions/locationId/currentLocation/currentLocationId/status/contact/contactMode/canContact/presenceNote）。角色引用规则与 `characterUpdates` 相同：id 优先，缺失或未命中时按当前场景唯一角色名匹配，重名或模糊不唯一则跳过 |
| `challengeUpdate` | 更新场景挑战（challengeId/id 优先；缺失或未命中时可按唯一 title/name/challengeTitle 或目标文本匹配；重名或模糊不唯一则跳过。字段包括 status/progress/strain/supports/evidenceAdd/evidenceIds/reason 等；完成时走挑战闸门、奖励、任务和阶段联动） |
| `evidenceAdd` | 添加证据（id/title/text/reliability/tags/supports/obtainedBy；同步知识、线索和探索奖励） |
| `revelationUpdate` | 更新关键结论（id/revelationId/conclusion/title/name/status/reason/newConclusion；影响主线和阶段推进）。id 优先；缺失或未命中时可按唯一 conclusion/title 匹配，重名或模糊不唯一则跳过。`conclusion` 可用于定位或创建新结论；更新既有结论正文应使用精确 id 或 `newConclusion` |
...
| `questsUpdate` | 更新任务（questId/questName/name/title + objectiveIdx/objectiveNumber/objectiveId/objectiveText/targetText/status/reason）。任务优先用 questId，也可按当前场景唯一任务名匹配；目标优先用 objectiveIdx/objectiveNumber，也可按唯一目标 id 或目标文本片段匹配。所有目标完成仍走 `WorldEngine.applyQuestUpdates()` 和 `completeQuestObjective()` 闸门、任务进展消息、任务奖励和防重复 |
| `itemAdd` | 添加物品（name/description/type/quantity/effects/uses/tags；走 `WorldEngine.grantInventoryItem()` 统一堆叠和事件日志） |
| `locationUpdate` | 新增或更新地点（id/name/description/connections/alertLevel），走 `WorldEngine.applyLocationUpdates()`；单条补丁最多 20 条、场景总地点最多 80 个；变化会写入事件日志并提示地图/局势 tab |

**禁止修改**：`settings`、`apiKey`、玩家属性、玩家 HP 等核心字段。补丁是"建议性"的，系统会先递归剥离敏感字段、原型污染键和非白名单顶层字段，限制数组/对象/字符串体积，再交给白名单逻辑应用。

结局后所有结构化补丁都是 no-op：`StrategyManager.applyStateUpdate()` 会先拦截，`WorldEngine.apply*Update()` 规则入口也会在非 `playing` 场景直接返回不变更结果，避免外部 agent 或未来 UI 误调用继续推进时钟、任务、剧情、线索、挑战、地图或势力。
...
| 完成任务目标 | `[quest_update:...]` 或 `questsUpdate` | 标记和补丁都可按任务名定位；结构化副本中不能绕过任务推进闸门 |
