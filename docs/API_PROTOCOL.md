...
<state_update>
{
  ...
  "npcAgendaUpdate": [{ "characterId": "char_xxx", "characterName": "塞拉斯", "currentPlan": "监控玩家接触过的证人" }],
...
| `npcAgendaUpdate` | 更新 NPC 日程（characterId/characterName/name/actorName + currentPlan/priority/schedule/offscreenActions）。角色引用规则与 `characterUpdates` 相同：id 优先，缺失或未命中时按当前场景唯一角色名匹配，重名或模糊不唯一则跳过 |
