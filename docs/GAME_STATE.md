...
- `heal/gold/exp/clock_delta/clock_resist/world_tension`：可作为背包直接使用效果，点击“使用”或输入“使用物品名”时立即结算。`heal`/`gold` 复用生命与经济规则入口，时钟效果优先按 `clockId`、`clockTag`、`clockName` 匹配，否则按物品标签匹配公开时钟。
