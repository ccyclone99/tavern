...
已解锁且仍有次数的 `companionResources` 会通过 `WorldEngine.getCompanionActionLeads()` 生成“请某某协助……”行动线索，并进入 `getCurrentSituation()`、推荐动作和 `getPreparationHints()`。未满足 `unlock` 或默认关系门槛的协助不会出现在这些线索中，避免提前公开 NPC 底牌；实际消耗仍发生在玩家合理请求、行动预览或检定结算时。`scope` 可为 `present`、`remote` 或 `pledged`，旧档缺失时按 `remote` 兼容；`present` 资源必须绑定 NPC 当前在场才会展示或进入检定资源，不能让暂时无关的角色跨场景帮忙。`remote`/`pledged` 资源只适合资料、背书、通讯、分析、打听或预先承诺，不会被用于潜行、强突和战斗等需要现场动作的检定。
