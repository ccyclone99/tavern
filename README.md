...
- [docs/UIUX_SPEC.md](docs/UIUX_SPEC.md)：下一阶段前端 UI/UX 优化规格（信息架构、输入模式、响应式、可访问性）
...
普通聊天适合对话和轻量互动；当你要做有风险、会改变局势或需要检定的事时，在输入区切到“行动”模式。

1. 输入明确行动，例如：“我想套出他隐瞒的货舱情报”。
2. 系统会先生成行动预览，显示行动类型、建议检定、风险、风险来源和失败后果。
3. 点击“确认行动”后，AI 才会按主持人规则结算；点击“取消”则不进入剧情。
4. 如果 AI 要求检定，输入区会出现检定卡，玩家点击“掷骰”后才生成结果；装备和非消耗物品会进入检定修正，消耗品会先显示为可用但不会自动扣除。

“行动”适合单次具体动作；“计策”适合多阶段计划、阴谋、拉拢、离间和长期目标。
...
- [ ] 切换“行动”模式，输入高风险行动，确认出现风险预览且取消不发送
- [ ] AI 要求 `[check:]` 时出现检定卡，点击“掷骰”后生成检定结果
- [ ] 点击左侧角色查看详情
