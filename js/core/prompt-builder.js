...
const PromptBuilder = {
    ...
    build(character, scene, messages, opts = {}) {
        ...
        const agendaBlock = this.buildNpcAgendaBlock(character, scene);
        ...
        const rules = this.buildRulesContext(scene, { audience: 'npc', character });
        if (rules) systemParts.push(rules);

        // 剧情弧（叙事骨架）
        const arcBlock = this.buildStoryArcContext(scene, { includePrivate: false });
    ...
    buildGroup(currentChar, scene, messages, allCharacters, opts = {}) {
        ...
        if (privateNpcBlock) systemParts.push(privateNpcBlock);
        const agendaBlock = this.buildNpcAgendaBlock(currentChar, scene);
        if (agendaBlock) systemParts.push(agendaBlock);
        ...
        // 规则层：属性 + 任务 + 地图 + 检定 + 动态事件 + 计策
        const rules = this.buildRulesContext(scene, { audience: 'npc', character: currentChar });
        if (rules) systemParts.push(rules);
...
- 玩家漫无目的时，用突发事件制造选择，给出主线、支线、探索或代价方向。

你可以通过以下标记触发游戏事件（放在回复末尾，每条标记独占一行）：
- 新角色登场：[new_char:角色名|emoji|公开外貌描写|公开性格特征|开场白|信条(可选)|价值排序(可选)|底线1、底线2(可选)|动机1、动机2(私密可选)|恐惧1(私密可选)|秘密1(私密可选)|筹码1、筹码2(私密可选)]
  私密字段只用于角色扮演和后续调查，不代表玩家已知；不要在正文旁白直接公开秘密、恐惧或筹码。
- 角色退场：[char_exit:角色名|原因]
- 新增任务：[quest:任务名|main或side|描述|目标1,目标2,...|奖励]
- 任务目标完成：[quest_update:任务名|目标序号(从1开始)]（结构化副本必须已有挑战、证据或结论支撑）
- 移动到新地点：[move:地点名]
- 触发剧情事件：[event:事件描述]
...
- 保持角色一致性`);
     ...
     */
    buildRulesContext(scene, options = {}) {
        const parts = [];
        const audience = String(options.audience || 'dm');
        const includeDmPrivate = options.includeDmPrivate === true || audience === 'dm' || audience === 'narrator';
        ...
        if (companionResources.length > 0) {
            const lines = companionResources.slice(0, 6).map(resource => {
                ...
                return `- ${resource.name}（${scopeLabel}协助${presenceText ? `，${presenceText}` : ''}）：${bits.join('、') || '叙事协助'}；剩余${resource.uses}次${resource.risk ? `；代价：${resource.risk}` : ''}`;
            });
        ...
        const clueBlock = includeDmPrivate
            ? this.buildClueGraphContext(scene)
            : this.buildVisibleClueContext(scene);
        if (clueBlock) parts.push(clueBlock);

        const failureBlock = includeDmPrivate
            ? this.buildFailureStateContext(scene)
            : this.buildVisibleFailureRiskContext(scene);
        if (failureBlock) parts.push(failureBlock);

        const gameplayBlock = this.buildGameplayFlowContext(scene, { includePrivate: includeDmPrivate });
        if (gameplayBlock) parts.push(gameplayBlock);

        const challengeBlock = this.buildChallengeContext(scene, { includePrivate: includeDmPrivate });
...
【生存系统】玩家有生命值(HP)、金币、等级。战斗、陷阱、跌落等会造成伤害；休息、治疗术、药水可恢复。系统已支持本地命令“休息”“使用某物”“购买补给/医疗包/零件包/短剑/护甲/工具包/扫描仪/伪装工具包/追踪工具包”；复杂治疗或交易仍可由你用剧情和标记处理。当涉及这些时，在回复末尾使用：
...
- [quest_update:任务名|目标序号] 标记某任务目标完成（结构化副本必须已有挑战、证据或结论支撑）
- [event:事件描述] 触发一次剧情事件
- [move:地点名] 建议移动到新地点
...
- [item_add:物品名|描述|类型|数量] 给予玩家物品`);
    ...
    buildPrivateNpcBlock(character) {
        ...
        if (Array.isArray(hiddenFacts) && hiddenFacts.length > 0) {
            npcIntelParts.push(`隐藏档案槽：\n${hiddenFacts.map(f => `- ${f.id} | ${f.type || 'fact'} | 标题：${f.title || ''} | ${f.truth || ''} | 暗示：${f.hint || ''}`).join('\n')}`);
        }
    ...
    buildVisibleClueContext(scene) {
        const unknowns = typeof WorldEngine !== 'undefined' && WorldEngine.getKnownUnknowns
            ? WorldEngine.getKnownUnknowns(scene)
            : (Array.isArray(scene?.clueGraph) ? scene.clueGraph : [])
                .filter(path => path && path.status !== 'hidden' && path.status !== 'confirmed')
                .map(path => {
                    const stages = Array.isArray(path.stages) ? path.stages : [];
                    const idx = Math.max(0, Math.min(Number(path.currentStage || 0), Math.max(0, stages.length - 1)));
                    const stage = stages[idx] || {};
                    return {
                        title: path.title,
                        status: path.status,
        ...
        const lines = (unknowns || [])
            .filter(item => item && (item.title || item.text))
            .slice(0, 6)
            .map(item => {
                const actions = Array.isArray(item.actions) && item.actions.length > 0
                    ? `\n  可追查：${item.actions.slice(0, 3).join('；')}`
                    : '';
                const evidence = Number(item.evidenceCount || 0) > 0 ? `；已有关联证据 ${item.evidenceCount} 条` : '';
                return `- ${item.title || '未命名线索'}（${item.status || item.level || 'hinted'}${evidence}）：${item.text || item.title}${item.source ? `（来源：${item.source}）` : ''}${actions}`;
            });
        if (lines.length === 0) return '';
        return `【玩家可见线索】\n${lines.join('\n')}\n\n线索边界：\n- 这里只是玩家已经接触到的线索面，不包含 DM 私密真相。\n- NPC 只能根据自己在场、自己的日程、公开事实和玩家已知内容回应；不要表现得知道全局真相。\n- 如玩家继续调查，可通过 knowledgeAdd 或 discoveryUpdate 逐步解锁，而不是直接揭露。`;
    },
    ...
    buildFailureStateContext(scene) {
        ...
        const lines = failures.slice(0, 8).map(f => {
            ...
            const counterplay = Array.isArray(f.counterplay) && f.counterplay.length
                ? ` 可缓解：${f.counterplay.slice(0, 3).join('；')}`
                : '';
            return `- ${f.title}（${f.status || 'armed'}）：${triggerText}。${f.hint || f.message || ''}${counterplay}`;
        }).join('\n');
    ...
    buildVisibleFailureRiskContext(scene) {
        const warnings = typeof WorldEngine !== 'undefined' && WorldEngine.getFailureWarnings
            ? WorldEngine.getFailureWarnings(scene)
            : [];
        const lines = (warnings || [])
            .filter(item => item && (item.text || item.title))
            .slice(0, 3)
            .map(item => {
                const progress = item.value !== undefined && item.max !== undefined
                    ? ` ${item.value}/${item.max}`
                    : '';
                const actions = Array.isArray(item.actions) && item.actions.length > 0
                    ? `\n  可缓解：${item.actions.slice(0, 3).join('；')}`
                    : '';
                return `- ${item.title || item.sourceName || '公开风险'}${progress}：${item.text || item.title}${actions}`;
            });
        if (lines.length === 0) return '';
        return `【玩家可见失败风险】\n${lines.join('\n')}\n\n风险边界：隐藏失败来源、隐藏时钟和私密真相不能直接公开；只能通过玩家已经看到的压力、可见反制或环境迹象来提示。`;
    },

    buildGameplayFlowContext(scene, options = {}) {
        ...
        const visibleRevelations = includePrivate
            ? revelations
            : revelations.filter(r => r && r.status !== 'unknown');
        const revLines = visibleRevelations.slice(0, 6).map(r =>
    ...
    buildChallengeContext(scene, options = {}) {
        const includePrivate = options.includePrivate !== false;
        let challenges = Array.isArray(scene?.sceneChallenges) ? scene.sceneChallenges : [];
        ...
        if (!includePrivate) {
            challenges = challenges.filter(ch => ch && ch.status === 'active');
        }
        if (challenges.length === 0) return '';
     ...
     */
    buildStateUpdateSchemaHint() {
        return `{ "strategies": { "create": [...], "update": [...] }, "knowledgeAdd": [], "discoveryUpdate": [{ "characterName": "NPC名", "factId": "secret_0_abcd", "state": "hinted" }], "intelAdd": [], "factionsUpdate": [], "characterUpdates": [{ "characterName": "NPC名", "trustDelta": 0, "suspicionDelta": 0 }], "clockUpdate": [], "storyArcUpdate": [], "storyPhaseUpdate": [], "clueUpdate": [{ "clueTitle": "线索链标题", "status": "suspected" }], "failureStateUpdate": [], "counterStrategyUpdate": [], "npcAgendaUpdate": [{ "characterName": "NPC名", "currentPlan": "...", "locationId": "地点id", "status": "present|nearby|remote|away", "contact": "in_person|voice|message|remote|none", "canContact": true }], "challengeUpdate": [], "evidenceAdd": [], "revelationUpdate": [{ "conclusion": "关键结论", "status": "confirmed" }], "flowGraphUpdate": {}, "questsUpdate": [{ "questName": "任务名", "objectiveText": "目标文本", "status": "completed" }], "itemAdd": [], "locationUpdate": [], "scene": { "worldTensionDelta": 0, "activeStrategyId": "" } }`;
    },
    ...
    buildStrategyProtocol(scene) {
        ...
        return `【计策主持人协议】\n你是一位主持人（DM），不替玩家做最终选择。当玩家提出目标、阴谋、调查、拉拢、离间、潜入、交易、威胁等意图时，应帮助创建或推进计策。\n\n${activeDesc}\n所有计策：\n${allStrategies}${strategyItemsBlock}\n\n规则：\n1. 信息不足时，最多追问 1-2 个关键问题（目标、筹码、风险偏好、关键 NPC）。\n2. 计划可执行时，推进阶段（intel → setup → action → complication → resolution）并给出风险值 0-100，同时记录 requiredIntel/usedIntel/exposure/counterplay。\n3. 每轮必须给玩家一个明确的下一步问题，或 2-3 个可选行动。\n4. 成功依赖玩家已知情报、筹码、关系、资源、检定和风险，不允许无代价成功；若玩家投入“可用计策物品”，应在 risk/exposure/usedIntel/resources 中写明物品名。计策进入 action/complication/resolution 或 executing/resolved/failed 后，可消耗物品会由系统扣除一次。\n5. 私密设定不是玩家已知；只有当玩家观察、调查、套话、取得证据或满足关系门槛时，才可以把它转化为 knowledgeAdd 或 discoveryUpdate。解锁隐藏档案优先写 factId；不知道 factId 时可写该 NPC 下唯一的 factTitle/type/hint。\n6. 后果必须具体影响关系、警觉、任务、资源、地点、时钟或世界局势。\n7. 计策状态包括：draft（草稿）、preparing（筹备中）、executing（执行中）、exposed（已暴露）、resolved（已解决）、failed（失败）。\n8. 当创建或更新计策、添加玩家已知情报、推进剧情弧/剧情阶段/线索/失败状态/挑战/证据/关键结论/时钟、调整 NPC 日程或反制时，在回复末尾追加隐藏状态补丁。关系和日程补丁优先写 characterId；不知道 id 时可写当前场景内唯一的 characterName/name：\n<state_update>\n${this.buildStateUpdateSchemaHint()}\n</state_update>\n补丁只包含你确认发生的变化，JSON 必须合法。玩家看不到补丁内容。`;
    },

    buildNpcAgendaBlock(character, scene = null) {
        if (!character?.agenda) return '';
        const agenda = character.agenda;
        const lines = [];
        if (scene && typeof WorldEngine !== 'undefined' && WorldEngine.getCharacterPresence) {
            const presence = WorldEngine.getCharacterPresence(scene, character.id);
            const place = presence.locationName || presence.locationId || '';
            if (presence.isPresent) lines.push(`当前位置：${place || '当前地点'}（在场）`);
            else if (place) lines.push(`当前位置：${place}（不在玩家当前地点）`);
            if (presence.canRemote && !presence.isPresent) lines.push(`联系方式：可远程联系`);
            if (!presence.canRemote && !presence.isPresent && presence.status !== 'unknown') lines.push(`联系方式：暂不可联系`);
        }
        ...
        if (lines.length === 0) return '';
        return `【NPC 个人日程，仅供扮演】\n${lines.join('\n')}\n\n规则：\n- 你有自己的计划，不会因为玩家不在场就停止行动。\n- 若玩家阻碍或利用你，你可以通过 counterStrategyUpdate 或 npcAgendaUpdate 反制；NPC 离开、转移地点或切断通讯时，应在 npcAgendaUpdate 写 locationId/status/contact/canContact。\n- 不要把这份日程作为旁白直接公开，除非玩家通过观察、调查、跟踪或对质获知。`;
    },
     ...
     */
    buildStoryArcContext(scene, options = {}) {
        const arcs = Array.isArray(scene.storyArcs) ? scene.storyArcs.filter(a => a && a.title) : [];
        if (arcs.length === 0) return '';
        const includePrivate = options.includePrivate !== false;
        if (!includePrivate) {
            const lines = arcs.slice(0, 4).map((arc, idx) => {
                const title = String(arc.publicTitle || arc.visibleTitle || arc.displayTitle || '').trim()
                    || `故事线${idx + 1}`;
                return `◆ ${title}（阶段：${arc.phase || '进行中'}）`;
            }
            ).join('\n');
            return `【剧情节奏 · 可感知层】\n${lines}\n\n推进边界：\n- 这里只提供当前故事正在展开的方向，不包含未来节拍、触发条件或私密真相。\n- NPC 可以主动制造可互动节点，但不能表现得知道自己不该知道的全局秘密。\n- 玩家偏离当前节奏时，提供支线、探索、对话或代价选择，而不是强制单一路线。`;
        }
        ...
        if (context.trigger === 'check_outcome') {
            ...
            if (lastCheck?.checkData) {
                ...
                const counterLines = Array.isArray(d.counterplayResults) && d.counterplayResults.length > 0
                    ...
                    : '';
                const resolvedConsequenceLines = Array.isArray(d.resolvedConsequences) && d.resolvedConsequences.length > 0
                    ? `\n- 解除后果：${d.resolvedConsequences.map(item => item.title || item.id || '后果').join('；')}`
                    : '';
                systemParts.push(`【当前任务】玩家刚刚进行了一次属性检定。请根据以下结果叙述具体后果：\n- 检定：${d.statName} ${d.roll} ${d.mod >= 0 ? '+' + d.mod : d.mod} = ${d.total} vs DC${d.dc}${d.baseDc && d.baseDc !== d.dc ? `（基础DC${d.baseDc}）` : ''}${resourceLines}${counterLines}${resolvedConsequenceLines}\n- 结果层级：${d.resultLabel || d.outcome || (d.success ? '成功' : '失败')}\n- 裁决提示：${d.consequenceHint || '按结果合理推进'}${consequenceLines}\n\n要求：\n- 大成功：给出额外收益、优势、机会或更深线索。\n- 成功：让目标按预期推进。\n- 部分成功：目标达成一部分，或达成但必须付出代价。\n- 失败推进：不要只写失败，必须产生新线索、新阻碍、新场景、资源损失、关系变化或反制。\n- 大失败：严重后果，但仍要打开新的剧情方向。`);
            } else {
